...
class Tokenizer:
    ...
    def __init__(self,
        ...
        if granularity == "word":
            ...
            if torch.cuda.is_available():
                self.ltp.to(torch.device("cuda"))
            else:
                self.ltp.to(torch.device("cpu"))
    ...
    def split_word(self, input_strings: List[str]) -> List:
        ...
        if self.segmented:
            # 预处理已分词的输入（如用空格分割）
            inputs = [s.split() for s in input_strings]
            # 调用 pipeline 处理已分词的输入
            output = self.ltp.pipeline(inputs, tasks=["cws", "pos"], is_preseged=True)
        else:
            # 直接对原始文本进行分词和词性标注
            output = self.ltp.pipeline(input_strings, tasks=["cws", "pos"])
        
        # 合并分词、词性标注和拼音
        result = []
        for words, tags in zip(output["cws"], output["pos"]):
            pinyin = [lazy_pinyin(word) for word in words]
            result.append(list(zip(words, tags, pinyin)))
        return result
